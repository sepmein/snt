...
Resource <- R6::R6Class(
  classname = "resource", public = list(
    ...
    download_to = NULL, initialize = function(
      ...
      if (is.null(private$check_local_file_type())) {
        stop(
          paste0(
          "Creating Resource Object: ", "Type of local file should be ",
      ...
      if (private$local_file_type == "raster") {
        ...
        stop(
          paste0(
          "Resource loading raster files", ", but local_file_type is not raster/shapefile/csv."
        )
        )
      }
    }, clean = function() {
  ...
  private = list(
    is_online = NULL, is_batch = NULL, local_file_type = NULL, check_local_file_type = function() {
      ...
      dest_file_path <- file.path(self$download_to, destfile)
      if (missing(api_url)) {
      ...
      if (download_method == "ftp") {
        ...
        if (file.exists(dest_file_path)) {
          ...
          stop(
          paste0(
            "File does not exists, ", "maybe download failed or user have removed file. ",
      ...
      if (download_method == "ftp") {
        ...
        filenames <- RCurl::getURL(api_url, ftp.use.epsv = FALSE, dirlistonly = TRUE)
      ...
      # Download FTP
      if (startsWith(api_url, "ftp")) {
        return("ftp")
      } else if (startsWith(api_url, "https")) {
        return("https")
      } else if (startsWith(api_url, "http")) {
        return("http")
      }
  ...
  "RasterResource", inherit = Resource, public = list(
    ...
    cores = NULL, initialize = function(
      ...
      if (!(private$local_file_type == "raster")) {
        stop(
          paste(
          "CountryShapeFile stack with raster ", ", resource file type should be raster"
        )
        )
      }
      ...
      if (typeof(self$local_destination) ==
        ...
        stop(
          "Local destination should either be a list of folder, a folder or a file name"
        )  # nolint
      }
      self$clean()
      ...
      # into shapefile
      extracted_raster_data <- raster::extract(raster_data, loaded_shp)
      # aggregate
      by_district <- self$get_district_raster_data(method, extracted_raster_data)
      # tibble
      ...
      if (file_test("-f", self$local_destination)) {
        ...
        if (length(palette) ==
          1) {
          # if palette is 'foo'
          palette <- rev(grDevices::hcl.colors(categories, palette))
        } else if (length(palette) >
          1) {
        ...
        if (self$target_adm_level == 3) {
          ...
          if (!is.null(self$adm2_shapefile)) {
          map <- map + tmap::tm_shape(adm2_shapefile) +
...
Rainfall <- R6::R6Class(
  classname = "Rainfall", inherit = RasterResource, public = list(
    ...
    download = function(target, path_to_save, start_date, end_date) {
    }, clean = function() {
      self$data <- self$data |>
        dplyr::mutate(
...
MAPPlasmodiumIndex <- R6::R6Class(
  "MAPPlasmodiumIndex", inherit = RasterResource, public = list(
    ...
    with_95CI = NULL, download_to = NULL, plasmodium_index = NULL, initialize = function(
      ...
      self$data <- self$data |>
        dplyr::mutate(year = stringr::str_extract(file, "\\d+(?=\\.\\w+$)"))
      ...
      # if with uncertainty self$data will be a list
