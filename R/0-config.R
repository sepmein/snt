...
use_relative_or_absolute <- function(root, relative) {
    ...
    if (!is.null(relative_path_type)) {
        return(relative)
    } else if (!is.null(combined_root_path_type)) {
...
#' @return NULL
#'
# generate a set of folders based on the configuration generated by config()
setup <- function(config) {
    # create the root folder
    dir.create(config$root)
    # create the input folder
    dir.create(config$root_input)
    # create the output folder
    dir.create(config$root_output)
    # create the raster folder
    dir.create(config$path_raster)
    # create the raster map folder
    dir.create(config$path_raster_map)
    # create the raster ihme folder
    dir.create(config$path_raster_ihme)
    # create the raster rainfall folder
    dir.create(config$path_raster_rainfall)
    # create the country folder
    dir.create(config$path_country)
    # create the routine folder
    dir.create(config$path_routine)
    # create the intervention folder
    dir.create(config$path_intervention)
    # create the dhs folder
    dir.create(config$path_dhs)
    # create the shapefile folder
    dir.create(config$path_shapefile)
}

config(
    country = "NGA",
    root = "/Users/sepmein/x/NGA",
    root_input = "input",
    root_output = "output",
    path_raster = "raster",
    path_raster_map = "map",
