# db connection ------- TODO remove this into env settings
db_name <- "malaria"
db_user <- "sepmein"
db_pass <- "crimson"
get_con <- function() {
...
constrain_indicators <- function(positional_index, ...) {
  ...
  hashes <- paste0("$", positional_index:(positional_index + ... - 1))
...
dbq_select <- function(
  ...
  if (!is.null(adm0)) {
    ...
    sql <- add_constrain(sql, constrain_adm0, index)
    index <- index + 1
  ...
  if (!is.null(year)) {
    ...
    sql <- add_constrain(sql, constrain_year, index)
    index <- index + 1
  ...
  if (!is.null(indicators)) {
    ...
    if (length == 1) {
      ...
      # example c('foo', 'bar')
      sql <- add_constrain(sql, constrain_indicators, index, length)
      index <- index + length
      query_list <- append(query_list, indicators)
    }
    modified <- TRUE
  ...
  if (!is.null(year_from) &&
    ...
    sql <- add_constrain(sql, constrain_range_year_included, index)
    index <- index + 2
    query_list <- append(query_list, c(year_from, year_to))
    modified <- TRUE
  }
  # sending query
  dbsq_select_adm0_year <- DBI::dbSendQuery(con, sql)
  if (modified) {
...
format_level <- function(level) {
  ...
  if (!(formatted %in% levels)) {
    stop(paste("level should be either", "adm0, adm1 or adm2"))
  }
  return(formatted)
}
#' @export
