...
aggregate_by <- function(admin_level, date_level) {
  # choose from admin_level, if adm1, then produce, adm1, if adm2, then produce adm1, adm2, if hf, then produce adm1, adm2, hf
  # choose from date_level, if year, then produce year, if month, then produce year, month
  ...
  if (admin_level == "adm1") {
    ...
    stop("admin_level must be one of adm1, adm2, hf, date_level must be one of year, month")
  }
}
