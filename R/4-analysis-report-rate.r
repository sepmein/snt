...
plot_hf_report_status <- function(report_status) {
  # TODO how to solve the problem of month is not included in args TODO how to
  # solve the problem of hf not included in args TODO solve the preset
  # argument problem, hfname, reported, year, month

  # make robust hfname, year and month are required check if hfname is present
  ...
  report_status[, `:=`(
    ...
    labs(title = "Report Status", y = "Health Facilities", x = "Date") +
...
plot_adm1_report_rate <- function(rep_rat) {
  ...
  ggplot(rep_rat) +
    ...
    labs(title = "Report Status By Adm1 and Date", y = "Districts", x = "Date") +
    theme_snt()
}
